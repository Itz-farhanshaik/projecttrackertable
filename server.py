...
class ProjectTrackerHandler(BaseHTTPRequestHandler):
    ...
    def handle_auth_login(self):
        ...
        if not is_valid_role(role):
            self.write_json(400, {"error": "Choose either task manager or team member."})
            return
    ...
    def handle_agent_request(self):
        ...
        system_instruction = "\n".join(
            [
                "You are Quantum Assistant, a Gemini-powered task manager agent.",
                "Answer using only the task data provided below. If the answer is not in the data, say that clearly instead of inventing details.",
                "Be concise, practical, and helpful.",
                "You can summarize blockers, deadlines, team structure, notes, bugs, and scheduled tasks.",
                "You do not directly edit the task from chat in this version; tell the user to use the manager tools in the app when they ask to change data.",
...
def main():
    ...
    print(f"Quantum Scope task manager Python server running at http://{HOST}:{PORT}")
